# type: ignore
"""
Model loading and processing module. Provide basic classes for loading models.
LoRA, VAE, and image annotation models.
"""
import json
...
try:
    ...
    Rearrange = None
    einops = None
    print(f"[A1rSpace] Warning: einops not available: {e}")
    traceback.print_exc()
...
class ModelLoaderBase:
    ...
    def load_checkpoint(ckpt_name):
        ...
        if folder_paths is None or comfy is None:
            raise ImportError("ComfyUI modules are not available")
    ...
    def load_taesd(name):
        ...
        encoder = next(filter(lambda a: a.startswith("{}_encoder.".format(name)), approx_vaes), None)
        decoder = next(filter(lambda a: a.startswith("{}_decoder.".format(name)), approx_vaes), None)
    ...
    def apply_lora_stack(cls, model, clip, lora_stack):
        ...
        for entry in entries:
            try:
                ...
                # 自适应 strength 处理
                # 优先使用 model_strength 和 clip_strength（如果存在）
                # 否则使用单一的 strength 值（向后兼容）
    ...
    def load_controlnet(control_net_name):
        ...
        Args:
            control_net_name: Name of the ControlNet file
            
        Returns:
            Loaded ControlNet model or None if loading fails
        """
        if folder_paths is None or comfy is None:
            raise ImportError("ComfyUI modules are not available")
        ...
        try:
            ...
            print(f"[A1r ControlNet Loader] Failed to load ControlNet '{control_net_name}': {e}")
            return None
    ...
    def apply_controlnet(positive, negative, image, control_net, strength, start_percent, end_percent, vae=None):
        ...
        Args:
            ...
            vae: Optional VAE for preprocessing
            
        Returns:
            Tuple of (positive, negative) conditioning with ControlNet applied
        """
...
class VisionModel(nn.Module):
    """
    Base class for vision models used in image tagging (JoyTag).
    
    Abstract base providing common interface for loading, saving, and
    training vision models for multi-label image classification tasks.
    """
    image_size: int
    n_tags: int
...
class CLIPMlp(nn.Module):
    """CLIP MLP (multi-layer perceptron) module."""
...
class SkipInit(nn.Module):
    """Skip initialization module with learnable scaling."""
...
class FastCLIPEncoderLayer(nn.Module):
    """Fast CLIP encoder layer with attention and MLP."""
    ...
    def __init__(
        ...
        # If activation class is missing (transformers not installed), try to import at runtime
        global QuickGELUActivation
        if QuickGELUActivation is None:
            try:
                from transformers.activations import QuickGELUActivation as _QG
                QuickGELUActivation = _QG
            except Exception:
                QuickGELUActivation = None

        act_cls = activation_cls if activation_cls is not None else QuickGELUActivation
        if act_cls is None:
            # Fallback to nn.GELU if QuickGELUActivation is unavailable
            act_cls = lambda: nn.GELU()

        self.mlp = CLIPMlp(hidden_size, 4 * hidden_size, act_cls)
...
class CLIPEmbeddingLayer(nn.Module):
    """CLIP-style patch embedding layer for vision transformers."""
...
class MHAPoolingHead(nn.Module):
    """Multi-head attention pooling head for aggregating sequence features."""
...
class GAPHead(nn.Module):
    """Global average pooling head for sequence to vector projection."""
...
class CLIPLikeModel(VisionModel):
    """CLIP-like vision model for image tagging with transformer architecture."""
