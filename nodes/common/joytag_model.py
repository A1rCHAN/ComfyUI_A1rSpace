...
# Optional: for loading safetensors
try:
...
class VisionModel(nn.Module):
    ...
    def load_model(path: Path | str, device: str | None = None) -> 'VisionModel':
        ...
        if (path / 'model.safetensors').exists() and load_file:
            resume = load_file(path / 'model.safetensors', device='cpu')
        else:
            resume = torch.load(path / 'model.pt', map_location=torch.device('cpu'))['model']
